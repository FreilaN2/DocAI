      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden border-t border-slate-200 dark:border-outline-variant/30 bg-white dark:bg-[#110e0c] overflow-hidden"
          >
            <div className="flex flex-col px-4 py-4 space-y-4">
              <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-2 ${location.pathname === '/' ? "text-primary-container font-bold" : "text-slate-600 dark:text-on-surface-variant font-medium"}`}>
                <span className="material-symbols-outlined text-[20px]">home</span>
                {t('navbar.home')}
              </Link>
              <Link to="/tools" onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-2 ${location.pathname === '/tools' ? "text-primary-container font-bold" : "text-slate-600 dark:text-on-surface-variant font-medium"}`}>
                <span className="material-symbols-outlined text-[20px]">construction</span>
                {t('navbar.tools')}
              </Link>
              <Link to="/upgrade" onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-2 ${location.pathname === '/upgrade' ? "text-primary-container font-bold" : "text-slate-600 dark:text-on-surface-variant font-medium"}`}>
                <span className="material-symbols-outlined text-[20px]">workspace_premium</span>
                {t('navbar.pricing')}
              </Link>
              <Link to="/support" onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-2 ${location.pathname === '/support' ? "text-primary-container font-bold" : "text-slate-600 dark:text-on-surface-variant font-medium"}`}>
                <span className="material-symbols-outlined text-[20px]">support_agent</span>
                {t('navbar.support')}
              </Link>
              {!user && (
                <div className="pt-2 mt-2 border-t border-slate-200 dark:border-outline-variant/30">
                  <Link to="/register" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center justify-center gap-2 bg-primary-container text-white font-bold py-3 rounded-lg shadow-sm hover:opacity-90 transition-all no-underline">
